...
class NodeParam ( QtCore.QObject ) :
	...
	def encodedTypeStr ( self ) : 
		...
		if self.type == 'link' :
			return self.linked_param.encodedTypeStr ()
	...
	def parseFromXML ( self, xml_param ) :
		#
		if not xml_param.attributes ().namedItem ( 'linked_node' ).isNull () :
			# linked_node here is just node id from nodegroup NodeNetwork
			# it should be resolved later to Node 
			self.linked_node = str ( xml_param.attributes ().namedItem ( 'linked_node' ).nodeValue () )
			print ( ':: Parameter is linked to node (id=%d)' % int ( self.linked_node ) )
		
		if not xml_param.attributes ().namedItem ( 'linked_param' ).isNull () :
			# linked_param here is just linked parameter name
			# it should be resolved later to NodeParam 
			self.linked_param = str ( xml_param.attributes ().namedItem ( 'linked_param' ).nodeValue () )
