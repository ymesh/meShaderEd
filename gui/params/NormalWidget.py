...
class Ui_NormalWidget_field ( object ) :
	...
	def setupUi ( self, NormalWidget ) :
		...
		self.selector = QtModule.QComboBox ( NormalWidget )
...
#
# Ui_NormalWidget_array
#
class Ui_NormalWidget_array ( object ) :
	#
	# setupUi
	#
	def setupUi ( self, NormalWidget ) :
	...
	def connectSignals ( self, NormalWidget ) :
		...
		for i in range ( self.widget.param.arraySize ) :
			if usePyQt4 :
				NormalWidget.connect ( self.controls [ i ][0], QtCore.SIGNAL ( 'editingFinished()' ), self.onFloatEditEditingFinished )
				NormalWidget.connect ( self.controls [ i ][1], QtCore.SIGNAL ( 'editingFinished()' ), self.onFloatEditEditingFinished )
				NormalWidget.connect ( self.controls [ i ][2], QtCore.SIGNAL ( 'editingFinished()' ), self.onFloatEditEditingFinished )
				NormalWidget.connect ( self.controls [ i ][3], QtCore.SIGNAL ( 'activated(int)' ), self.onCurrentIndexChanged )
			else :
	...
	def disconnectSignals ( self, NormalWidget ) :
		...
		for i in range ( self.widget.param.arraySize ) :
			if usePyQt4 :
				NormalWidget.disconnect ( self.controls [ i ][0], QtCore.SIGNAL ( 'editingFinished()' ), self.onFloatEditEditingFinished )
				NormalWidget.disconnect ( self.controls [ i ][1], QtCore.SIGNAL ( 'editingFinished()' ), self.onFloatEditEditingFinished )
				NormalWidget.disconnect ( self.controls [ i ][2], QtCore.SIGNAL ( 'editingFinished()' ), self.onFloatEditEditingFinished )
				NormalWidget.disconnect ( self.controls [ i ][3], QtCore.SIGNAL ( 'activated(int)' ), self.onCurrentIndexChanged )
			else :
