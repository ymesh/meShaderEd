...
#define snoiset(p,t) (2 * (float noise(p,t)) - 1)
#define snoisexy(x,y) (2 * (float noise(x,y)) - 1)
#define vsnoise(p) (2 * (vector noise(p)) - 1)
#define vsnoiset(p,t) (2 * (vector noise(p,t)) - 1)
#endif
...
#define filteredsnoiset(p,width,t) \
    (snoiset(p,t) * (1-smoothstep (0.2,0.75,width)))
#define filteredvsnoiset(p,width,t) \
    (vsnoiset(p,t) * (1-smoothstep (0.2,0.75,width)))
